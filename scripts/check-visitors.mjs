...
assert.ok(routes.length>10&&routes.length<=61,'Bound crowd density across multiple paths');
const courts=data.features.filter(f=>f.kind==='court').map(f=>courtFrame(f.points));
let hidden=false,inbound=false,outbound=false;
for(let r=0;r<routes.length;r++){
  ...
  for(let t=0;t<180;t+=.17){
    ...
    if(route.entrance){hidden ||= !f.visible;inbound ||= next.z<f.z;outbound ||= next.z>f.z;}
    else {
      assert.equal(pathIntervals([f.x-.01,f.z],[f.x+.01,f.z],courts,.2).length,1,'Visitors stay off playing enclosures');
      assert.ok(!data.buildings.some(b=>b.base<2&&pointInRing([f.x,f.z],b.ring)),'Visitors stay outside buildings');
    }
  }
}
assert.ok(hidden&&inbound&&outbound,'Entrance flow includes arrivals, concealed turnaround and departures');
console.log(`Visitor checks passed: ${routes.length*4} visitors; continuous motion, clear court/building routes and entrance flow.`);
